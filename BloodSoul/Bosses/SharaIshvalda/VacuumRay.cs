...
{
    public class VacuumRay : ModProjectile
    {
        ...
        {
            DisplayName.SetDefault("Õæ¿ÕÖ®Êø");
        }
        ...
        public override void AI()
        {
            ...
            if (AITimer == 80 && !Main.dedServ)
                SoundEngine.PlaySound(SoundID.DD2_BetsyFlyingCircleAttack, Projectile.position);
        ...
        public void DrawLaser(Texture2D texture, Vector2 start, Vector2 unit, float rotation = 0f, float scale = 1f, float maxDist = 2000f, Color color = default, int transDist = 1)
        {
            ...
            for (float i = transDist; i <= (maxDist * (1 / LaserScale)); i += LaserSegmentLength)
            {
                ...
                Main.EntitySpriteDraw(texture, origin - Main.screenPosition + new Vector2(0, Projectile.gfxOffY),
                    new Rectangle((int)(256 * Frame), LaserEndSegmentLength, 80, LaserSegmentLength), color, r,
                    new Vector2(80 / 2, LaserSegmentLength / 2), scale, 0, 0);

                Main.EntitySpriteDraw(texture, origin - Main.screenPosition + new Vector2(0, Projectile.gfxOffY),
                    new Rectangle((int)(256 * Frame), LaserEndSegmentLength, 80, LaserSegmentLength), color * 0.5f, r,
            ...
            Main.EntitySpriteDraw(texture, start + unit * (transDist - LaserEndSegmentLength) - Main.screenPosition + new Vector2(0, Projectile.gfxOffY),
                new Rectangle((int)(256 * Frame), 0, 80, LaserEndSegmentLength), color, r, new Vector2(80 / 2, LaserSegmentLength / 2), scale, 0, 0);
            // Draws the Laser 'end'
            Main.EntitySpriteDraw(texture, start + maxDist * (1 / scale) * unit - Main.screenPosition + new Vector2(0, Projectile.gfxOffY),
                new Rectangle((int)(256 * Frame), LaserSegmentLength + LaserEndSegmentLength, 80, LaserEndSegmentLength), color, r, new Vector2(256 / 2, LaserSegmentLength / 2), scale, 0, 0);

            // Draws the Laser 'base'
            Main.EntitySpriteDraw(texture, start + unit * (transDist - LaserEndSegmentLength) - Main.screenPosition + new Vector2(0, Projectile.gfxOffY),
                new Rectangle((int)(256 * Frame), 0, 80, LaserEndSegmentLength), color * 0.5f, r, new Vector2(80 / 2, LaserSegmentLength / 2), scale * new Vector2(pulse, 1), 0, 0);
            // Draws the Laser 'end'
            Main.EntitySpriteDraw(texture, start + maxDist * (1 / scale) * unit - Main.screenPosition + new Vector2(0, Projectile.gfxOffY),
                new Rectangle((int)(256 * Frame), LaserSegmentLength + LaserEndSegmentLength, 80, LaserEndSegmentLength), color * 0.5f, r, new Vector2(80 / 2, LaserSegmentLength / 2), scale * new Vector2(pulse, 1), 0, 0);
        }
