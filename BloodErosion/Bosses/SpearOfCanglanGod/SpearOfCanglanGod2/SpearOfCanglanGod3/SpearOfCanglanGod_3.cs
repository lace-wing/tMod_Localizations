...
namespace BloodErosion.NPCs.Bosses.SpearOfCanglanGod.SpearOfCanglanGod2.SpearOfCanglanGod3
{
    [AutoloadBossHead]
    class SpearOfCanglanGod_3 : FSMnpc
    {
        ...
        {
            DisplayName.SetDefault("Dark blue evil spirit thunder spear · LAN");
            DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "苍煞雷矛·岚");
        ...
        public override void SetDefaults()
        {
            ...
            NPC.aiStyle = -1;
            NPC.scale = 1f;
            NPC.lifeMax = 300000 / 3;
            NPC.defense = 55;
            NPC.damage = 215 / 3;
            BossBag = ModContent.ItemType<SpearOfCanglanGodItem>();
            if (!Main.dedServ)
            {
                Music = MusicLoader.GetMusicSlot(Mod, "Sounds/Music/Spear2");
            }
        }
        public override void ModifyNPCLoot(NPCLoot npcLoot)
        {
            npcLoot.Add(ItemDropRule.BossBag(BossBag));
            npcLoot.Add(ItemDropRule.MasterModeCommonDrop(ModContent.ItemType<SpearOfCanglanGodRelicItem>()));
        }
            ...
            switch ((SpearOfCanglanGodAI)State1)
            {
                case SpearOfCanglanGodAI.HeadLighting:
                {
                        ...
                        if (Time2 >= 300)
                        {
                            ...
                            Time2 = 0;
                            SwitchState2(0);
                            SwitchState1((int)SpearOfCanglanGodAI.ContinuousLightning, (int)SpearOfCanglanGodAI.ThunderCircle + 1);
                        }
                        break;
                }
                            ...
                            case 1:
                                {
                                    ...
                                    if (Time1 > 30)
                                    {
                                        ...
                                        if (Time2 > 4)
                                        {
                                            ...
                                            Time2 = 0;
                                            SwitchState2(0);
                                            SwitchState1((int)SpearOfCanglanGodAI.HeadLighting, (int)SpearOfCanglanGodAI.ThunderCircle + 1);
                                        }
                                        else
                                        {
                                            NPC.velocity *= 0f;
                                            ...
                                            for (int i = 0; i < 20; i++)
                                            {
                                                ...
                                                Projectile.NewProjectile(Source_NPC, NPC.Center, r * 1.2f, TL2, 155 / 3, 0f, Main.myPlayer);
                                                interval++;
                                            }
                                            SwitchState2(0);
                                        }
                ...
                case SpearOfCanglanGodAI.ThunderCircle:
                    {
                        Time1++;
                        NPC.velocity *= 0;
                        switch (Time1)
                            ...
                            case 120:
                                {
                                    if (Main.netMode != 1)
                                    {
                                        ...
                                        {
                                            Vector2 center = Target.Center + (r.ToRotationVector2() * 600);
                                            var proj = Projectile.NewProjectileDirect(NPC.GetSpawnSourceForNPCFromNPCAI(),
                                                center, Vector2.Normalize(Target.Center - center) * 5, TL2,
                                                145 / 3, 2.3f, Main.myPlayer);
                                            SoundEngine.PlaySound(SoundID.DD2_BookStaffCast, NPC.position);
                                            proj.timeLeft = 1000;
                            ...
                            case 240:
                                {
                                    if (Main.netMode != 1)
                                    {
                                        ...
                                        SoundEngine.PlaySound(SoundID.Thunder, NPC.position);
                                        Projectile.NewProjectile(NPC.GetSpawnSourceForNPCFromNPCAI(), targetOldPos, ToTarget * 0f, L, 180 / 3, 0, Main.myPlayer, 0, 0);
                                        for (float r = 0; r < MathHelper.TwoPi; r += MathHelper.TwoPi / 24)
                                        {
        ...
        public override void OnKill()
        {
            ...
            NPC.active = false;
            SoundEngine.PlaySound(SoundID.Thunder, NPC.position);
            Projectile.NewProjectile(projectileSource, NPC.Center.X, NPC.Center.Y, 0, 0, ModContent.ProjectileType<LightningProjectile2>(), 0, 0, 0);
            base.OnKill();
        }
