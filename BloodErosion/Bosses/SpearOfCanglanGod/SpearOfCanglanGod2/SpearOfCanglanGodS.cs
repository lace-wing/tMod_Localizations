...
{
    class SpearOfCanglanGodS : FSMnpc
    {
        float r = 0;
        ...
        {
            DisplayName.SetDefault("Divine spear thunder shadow");
            DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "神矛雷影");
        ...
        public override void AI()
        {
            ...
            int ThunderLightning = ModContent.ProjectileType<ThunderLightning>();
            if (Target.dead)
            {
                NPC.life = 0;
                return;
            ...
            if (Main.rand.Next(200) == 0 && Main.netMode != 1)
            {
                ...
                Projectile.NewProjectile(NPC.GetSpawnSourceForNPCFromNPCAI(), center, (Target.Center - center).SafeNormalize(Vector2.UnitX) * 5
                    , ThunderLightning, 165 / 3, 2f, Main.myPlayer);
            }
            #endregion
        }
