...
{
    public class IceBlock : BaseProj
    {
        private Vector2 OldVec = Vector2.Zero;
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("IceBlock");
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = -20; i <= 19; i++)
            {
                float r2 = r + i * MathHelper.Pi / 20f;
                Vector2 shootVel = r2.ToRotationVector2() * -15;
                Terraria.Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Center, shootVel, 174, 135 / 6, 10, NPC.whoAmI);
            }
            SoundEngine.PlaySound(SoundID.Item62, Projectile.position);
            for (int i = 0; i < 25; i++)
            {
                Dust.NewDust(Projectile.position + Projectile.velocity, Projectile.width, Projectile.height, DustID.Ice, Projectile.oldVelocity.X * 0.2f, Projectile.oldVelocity.Y * 0.2f);
            }
        }
    }
}
